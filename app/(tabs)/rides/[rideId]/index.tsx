...
import { View, Text, StyleSheet, ScrollView, Alert, ActivityIndicator, Dimensions, TouchableOpacity } from 'react-native'; // Added TouchableOpacity
import { Stack, useLocalSearchParams, useRouter } from 'expo-router';
import MapView, { Marker } from 'react-native-maps'; // PROVIDER_GOOGLE removed
...
import { OSM_TILE_URL } from '../../../../constants/mapConstants'; // Import OSM tile URL
...
const RideDetailScreen = () => {
  ...
  const getMapRegion = () => {
    ...
    const latDelta = Math.abs(startLat - endLat) * 1.8; // Increased multiplier for more zoom out
    const lngDelta = Math.abs(startLng - endLng) * 1.8;

    return {
      latitude: midLat,
      longitude: midLng,
      latitudeDelta: Math.max(latDelta, 0.03), // Ensure a minimum delta
    ...
    <ScrollView style={styles.container}>
      {ride.startLocation?.coordinates && ride.endLocation?.coordinates && (
          <View style={styles.mapContainer}>
            <MapView
              style={styles.map}
              urlTemplate={OSM_TILE_URL} // Use OSM tiles
            ...
            >
              <Marker
                ...
                pinColor={COLORS.success} // May not be effective with default OSM markers
              />
              <Marker
                ...
                pinColor={COLORS.primary} // May not be effective with default OSM markers
              />
            </MapView>
      ...
      <View style={styles.detailSection}>
        ...
        <TouchableOpacity onPress={() => router.push(`/(tabs)/rides/user-profile/${ride.driver._id}`)}>
      ...
      {ride.passengers && ride.passengers.length > 0 && (
        <View style={styles.detailSection}>
            ...
            {ride.passengers.map(p => (
                <TouchableOpacity key={p._id} onPress={() => router.push(`/(tabs)/rides/user-profile/${p._id}`)}>
      ...
      {isDriverOfThisRide && ride.status === 'pending' && (
        <>
         ...
         <AppButton title="View Booking Requests" onPress={() => router.push(`/(tabs)/rides/${ride._id}/bookings`)} color="info" style={styles.actionButton}/>
        </>
      )}
      ...
      {ride.status === 'completed' && user && (
        <AppButton 
            ...
            onPress={() => {
                ...
                const wasPassenger = ride.passengers.find(p => p._id === user._id);
                ...
                if (isDriverOfThisRide) {
                    if (ride.passengers.length === 1) {
                        ...
                        reviewType = 'rider_review';
                         router.push({ pathname: '/(tabs)/rides/add-review', params: { rideId: ride._id, revieweeId, reviewType, revieweeName }});
                    } else if (ride.passengers.length > 1) {
                        Alert.alert("Review Passenger", "Which passenger would you like to review?", [
                            ...ride.passengers.map(p => ({
                                text: p.name,
                                onPress: () => router.push({ pathname: '/(tabs)/rides/add-review', params: { rideId: ride._id, revieweeId: p._id, reviewType: 'rider_review', revieweeName: p.name }})
                            })),
                    ...
                    // This case should ideally not happen if button logic is correct
                    // Alert.alert("Info", "You cannot review for this ride.");
                }
            }} 
            color="warning" 
...
const styles = StyleSheet.create({
  ...
  mapContainer: { // Added container for map and attribution
  ...
  osmAttributionSmall: { // For attribution directly under the map
    fontSize: 9,
    textAlign: 'center',
    paddingVertical: 3,
  ...
  detailSection: {
    ...
    padding: 20,
    // marginBottom: 10, // Removed to make attribution closer or part of this section
    borderBottomWidth: 1,
