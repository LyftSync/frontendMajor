...
import { Modal, View, StyleSheet, Text, TextInput, FlatList, TouchableOpacity, Alert, ActivityIndicator, Keyboard } from 'react-native';
import MapView, { Marker } from 'react-native-maps';
import { COLORS } from '../../constants/colors';
import AppButton from '../UI/AppButton';
import { Ionicons } from '@expo/vector-icons';
import { DEFAULT_MAP_REGION, NOMINATIM_BASE_URL, OSM_TILE_URL } from '../../constants/mapConstants';
...
async function fetchAndParseNominatim(url: string, operationName: string) {
  ...
  if (!response.ok) {
    console.error(`Nominatim ${operationName} error: Status ${response.status}`);
    console.error(`Nominatim ${operationName} response: ${responseText}`);
    throw new Error(`Nominatim ${operationName} request failed with status ${response.status}`);
  }
  ...
  try {
    ...
    console.error(`Nominatim ${operationName} raw response that failed parsing: ${responseText}`);
    throw new Error(`Failed to parse Nominatim ${operationName} response as JSON.`);
  }
}


const MapPickerModal: React.FC<MapPickerModalProps> = ({ isVisible, onClose, onLocationSelect, initialRegion }) => {
  ...
  useEffect(() => {
    if (isVisible) {
      if (initialRegion) {
        ...
        setMarkerCoordinate({latitude: initialRegion.latitude, longitude: initialRegion.longitude});
        reverseGeocodeNominatim(initialRegion.latitude, initialRegion.longitude);
      } else {
  ...
  const ensureRateLimit = async () => {
    ...
    if (timeSinceLastRequest < 1100) { // Slightly more than 1 second
  ...
  const reverseGeocodeNominatim = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      if (data && data.display_name) {
        ...
        console.warn('Nominatim reverse geocoding failed (no display_name):', data);
      }
    } catch (error) {
  ...
  const handleSearchNominatim = async (text: string) => {
    ...
    debounceTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        await ensureRateLimit();
        const url = `${NOMINATIM_BASE_URL}/search?q=${encodeURIComponent(text)}&format=json&addressdetails=1&limit=5&countrycodes=IN&accept-language=en`;
  ...
  return (
    <Modal visible={isVisible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.container}>
        ...
        />
        {(loading && searchResults.length === 0 && !isDebouncing) && <ActivityIndicator style={styles.listLoading} color={COLORS.primary}/>}
        ...
        {searchResults.length > 0 && (
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.resultItem} onPress={() => handleSelectSearchResultNominatim(item)}>
        ...
        >
          {markerCoordinate && <Marker coordinate={markerCoordinate} draggable 
            onDragEnd={(e) => {
        ...
        {selectedAddress ? (
          ...
          <Text style={styles.addressText}>Tap on map or search to select location</Text>
        )}
         {loading && markerCoordinate && <ActivityIndicator style={{marginVertical: 5}} color={COLORS.primary}/>}
        
        <Text style={styles.osmAttribution}>Map data © OpenStreetMap contributors</Text>
        <AppButton title="Confirm Location" onPress={confirmLocation} style={styles.confirmButton} />
      </View>
    </Modal>
  );
};
